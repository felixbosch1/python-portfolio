...
def getDogSize():
    ...
    if x == "tiny":
        ...
        for i in range(len(dog_breeds)):
            if 11 < dog_weights[i] <= 25:
                filtered_list.append(dog_breeds[i])
    elif x == "medium":
        for i in range(len(dog_breeds)):
            if 25 < dog_weights[i] <= 60:
                filtered_list.append(dog_breeds[i])
    elif x == "large":
        for i in range(len(dog_breeds)):
            if dog_weights[i] > 60:
...
def dogrecsize():
    getDogSize()
    print("We recommmend that you get a " + random.choice(filtered_list))
    filtered_list.clear()
def open_image(images):
    response = requests.get(images)
    img = Image.open(BytesIO(response.content))
    img.show()
def doginfo():
    breed = input("What dog breed would you like to get more infomation about?")
...
def purpose():
    trait = input("What do you want the purpose of your new dog to be?")
    trait = trait.lower()
    try:
        for i in range(len(dog_breeds)):
            if trait in bredfor[i]:
                filtered_list.append(dog_breeds[i])
        return(filtered_list)
    except:
        print("Your search could not be fulfilled")
def dogrecpurp():
    purpose()
    print("We recommmend that you get a " + random.choice(filtered_list))
    filtered_list.clear()
def menu():
    while True:
        print("""Menu:
1. Info on certain dog
2. Filter for purpose
3. Filter for size
4. Filtered list
5. All breeds
6. Recommender
7. Quit""")
        option = input("What option would like to do: (1-7) ")
        if option == "1":
            doginfo()
        elif option == "2":
            purpose()
            ...
            rec = input("Would you like your recommendation based off size or purpose: ")
            rec = rec.lower()
            if rec == "purpose":
                dogrecpurp()
            elif rec == "size":
